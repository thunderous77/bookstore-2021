...
* 通过固定每个信息的长度储存各种信息
* 登录用户信息存内存，其他信息存外存
* A文档存账户信息，A1索引文档（ID->A中ID对应信息位置（一个指针））
* B文档存书本信息，B1文档作为索引文件（ISBN->书本序号（1-base，乘信息长度后可直接访问书本信息）），B2-B4索引文件（Book-Name/Author/Keyword->书本序号）
* C文档存交易信息
* D 文档存所有系统操作信息（用于生成日志）
...
* 读入类：处理每一行输入，返回账户/图书/日志类和这一行（参数建议到具体类成员函数里处理）
...
* 账户类：成员函数对应每个账户系统指令，可自行添加辅助函数（如文件读取），下同
...
* 登录指令
  ...
  * 文件里遍历找User-ID,先判断等级，等级不够再判断对应的Password和输入的一不一样
...
* 删除账户
  ...
  * 遍历文件ID，不存在则操作失败，再遍历登录的账户ID，不存在就删除，否则操作失败
...
* B1索引文档信息存储
  ...
  * sign：数组里ISBN的最小值（x不固定，要保证数组内元素个数在（sqrt（n），2sqrt（n））之间，n是书本数，超过了就把数组分成两段并在之后增加一个节点）
  * 数组：ISBN+图书编号
  * 查找ISBN时先比较sign确定范围，再遍历数组得到图书编号，即可访问B中图书信息，复杂度应该是sqrt（n）
  
* B2-B4索引文档信息储存
  * 类似B1
  * sign是数组里Book-Name/Author/Keyword的最小值
